...
const Dashboard = () => {
  const { fecha, setFecha } = useContext(AuthContext)
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await fetch(`http://localhost:3001/graficos/fecha/${fecha}`);
        ...
        if (result.length > 0) {
          ...
          setData({
            total_ganancias_netas: parseFloat(totales.total_ganancias_netas),
            total_ganancias_brutas: parseFloat(totales.total_ganancias_brutas),
            total_perdidas: parseFloat(totales.total_perdidas),
